...
class TestVerification(FrappeTestCase):
	...
	def test_failed_verification(self):
		...
		self.assertEqual(frappe.get_doc("EdgePay Payment Transaction", txn_name).status, "Failed")
	...
	def test_idempotent_repeat_verification(self):
		...
		self.assertEqual(res2["transaction"], res1["transaction"])
		txns = frappe.get_all(
			"EdgePay Payment Transaction", filters={"payment_request": self.payment_request.name}
		)
		self.assertEqual(len(txns), 1)
	...
	def test_terminal_business_status_blocks_verification(self):
		for status in ["Expired", "Cancelled", "Refunded"]:
			...
			with self.assertRaises(frappe.ValidationError) as context:
				verify_transaction(self.payment_request.name)
			self.assertIn(
				"cannot verify transaction for a payment request with final status",
	...
	def test_secrets_redacted_in_raw_response(self):
		...
		raw_resp = json.loads(frappe.get_doc("EdgePay Payment Transaction", txn_name).raw_response_json)
