...
frappe.pages["edgepay-integrations"].on_page_show = function (wrapper) {
	...
	frappe.require("edgesuite_ui.bundle.js", () => {
		...
		frappe.call({ method: "edgepayv1.api.operations.get_integrations_context", callback: (response) => render(response.message || {}), error: (error) => { root.innerHTML = `<div class="alert alert-danger">${esc(error?.message || String(error))}</div>`; } });
	});
};
