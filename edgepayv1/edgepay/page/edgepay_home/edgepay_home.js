...
frappe.pages["edgepay-home"].on_page_show = function (wrapper) {
	...
	const displayCount = (value) =>
		value === null || value === undefined ? __("Restricted") : value;
	...
	const render = (context) => {
		...
		const rows = recent.length
			? recent
					...
					.join("")
			: `<tr><td colspan="6" class="text-center text-muted p-4">${escape(
					__("No payment requests are available for this merchant.")
			  )}</td></tr>`;
		...
		const merchantBlock = merchant.visible
			? `
				<div class="card-body">
					...
					<div class="row mt-3">
						<div class="col-md-3"><div class="text-muted small">${escape(
							__("Onboarding")
						)}</div><strong>${escape(merchant.onboarding_status || "—")}</strong></div>
						<div class="col-md-3"><div class="text-muted small">${escape(
							__("Verification")
						)}</div><strong>${escape(
					merchant.verification_status || "—"
						...
						<div class="col-md-3"><div class="text-muted small">${escape(
							__("Currency")
						)}</div><strong>${escape(merchant.default_currency || "—")}</strong></div>
						<div class="col-md-3"><div class="text-muted small">${escape(
							__("Live payments")
						)}</div><strong>${escape(
					merchant.live_payments_allowed ? __("Approved") : __("Not approved")
			  )}</strong></div>
					</div>
				</div>`
			: `
				<div class="card-body">
					...
					<p class="text-muted mb-0">${escape(
						__(
							"Assign an active EdgePay Merchant User membership before reviewing merchant payment operations."
						)
					)}</p>
				</div>`;
		...
		root.innerHTML = `
			<div class="d-flex justify-content-between align-items-start gap-3 flex-wrap mb-4">
				<div>
					...
					<p class="text-muted mb-0">${escape(
						__(
							"Review merchant-scoped payment activity, onboarding and integration readiness without exposing provider or API secrets."
						)
					)}</p>
				</div>
				<div class="d-flex gap-2">
					<a class="btn btn-default btn-sm" href="/app/merchant-onboarding">${escape(
						__("Merchant Onboarding")
					)}</a>
					<a class="btn btn-primary btn-sm" href="/app/edgepay-payment-request">${escape(
						__("Payment Requests")
					)}</a>
				</div>
			</div>
			...
			<section id="payment-summary" class="mb-4">
				<div class="d-flex justify-content-between align-items-center mb-2">
					...
					<div class="text-muted small">${escape(
						merchantContext.merchant_branch || merchantContext.merchant_account || ""
					)}</div>
				</div>
				<div class="row">${countCards}</div>
				<div class="text-muted small">${escape(__("Attempts"))}: ${escape(__("Successful"))} ${escape(
			attempts.Successful || 0
		)} · ${escape(__("Pending"))} ${escape(attempts.Pending || 0)} · ${escape(
			__("Failed")
		)} ${escape(attempts.Failed || 0)}</div>
			</section>

			<section id="registration-readiness" class="card mb-4">
				<div class="card-body">
					<div class="d-flex justify-content-between align-items-start mb-3 gap-3">
						<div><h4 class="mb-1">${escape(
							__("Merchant activation readiness")
						)}</h4><p class="text-muted mb-0">${escape(
			readiness.message || ""
		)}</p></div>
					...
					<ul class="list-group list-group-flush">${
						...
						`<li class="list-group-item text-muted">${escape(
							__("No readiness checks are available.")
						)}</li>`
					}</ul>
				</div>
			</section>
				...
				<div class="card-body">
					...
					<div class="row">
						<div class="col-md-4"><div class="text-muted small">${escape(
							__("Active provider accounts")
						)}</div><strong>${escape(
			displayCount(operational.active_provider_accounts)
		)}</strong></div>
						<div class="col-md-4"><div class="text-muted small">${escape(
							__("Enabled API clients")
						)}</div><strong>${escape(
			displayCount(operational.enabled_api_clients)
		)}</strong></div>
						<div class="col-md-4"><div class="text-muted small">${escape(
							__("Enabled delivery endpoints")
						)}</div><strong>${escape(
			displayCount(operational.enabled_delivery_endpoints)
		)}</strong></div>
					</div>
					<p class="text-muted small mt-3 mb-0">${escape(
						__(
							"Provider credentials, API client secrets, webhook signing secrets, raw provider payloads and live-call control fields are intentionally excluded."
						)
					)}</p>
				</div>
			</section>
			...
			__("Amount")
		)}</th><th>${escape(__("Paid"))}</th><th>${escape(__("Status"))}</th><th>${escape(
			__("Source")
		)}</th></tr></thead>
						<tbody>${rows}</tbody>
					</table></div>
