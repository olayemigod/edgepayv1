...
frappe.pages["edgepay-finance"].on_page_show = function (wrapper) {
	...
	const renderNoContext = (state) => {
		...
		root.innerHTML = `<section class="card"><div class="card-body p-4"><h3 class="mb-2">${esc(__("Merchant context required"))}</h3><p class="text-muted">${esc(state.message || __("A merchant must be assigned before finance operations can be used."))}</p>${action}</div></section>`;
	};
	...
	frappe.require("edgesuite_ui.bundle.js", () => {
		...
		frappe.call({
			method: "edgepayv1.api.operations.get_finance_context",
