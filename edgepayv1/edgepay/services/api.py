...
def validate_live_provider_readiness(provider_name):
	try:
		...
		api_key_present = bool(provider_doc.api_key and provider_doc.get_password("api_key"))
		secret_key_present = bool(provider_doc.secret_key and provider_doc.get_password("secret_key"))
		return {
...
def handle_checkout_callback(payment_request=None, provider_reference=None, transaction_reference=None, status=None):
	...
	try:
		...
		return {"ok": False, "status": "error", "message": "An error occurred during verification", "data": None}
...
def mark_payment_handoff_delivered(event_name):
	try:
		...
		return {"ok": True, "status": "success", "message": f"Handoff event {event_name} marked as delivered", "data": None}
	except Exception as exc:
		return _error_response(exc)
...
def mark_payment_handoff_failed(event_name, error_message=None):
	try:
		...
		return {"ok": True, "status": "success", "message": f"Handoff event {event_name} marked as failed", "data": None}
	except Exception as exc:
		return _error_response(exc)
