		...
		frappe.require('edgeui.bundle.js', () => {
			...
			if (!runtime?.createEdgeApp || missing.length) {
				...
				showFailure(
					page,
					missing.length
						? __('EdgePay requires EdgeSuite UI 0.6.3 or newer. Missing: {0}', [missing.join(', ')])
			...
			frappe.require('edgepay_workspace.bundle.js', () => {
				...
				if (typeof window.mountEdgePayWorkspace !== 'function') {
					loading.remove();
					showFailure(page, __('The EdgePay EdgeSuite workspace bundle failed to register.'));
					return;
				}
				try {
					loading.remove();
					const root = $('<div class="edgepay-workspace-root" data-edge-product="edgepay"></div>').appendTo(page.body);
					wrapper.vue_app = window.mountEdgePayWorkspace(root[0], { mode });
				} catch (error) {
					console.error('Error mounting EdgePay workspace:', error);
					loading.remove();
					showFailure(page, __('Error mounting EdgePay workspace: {0}', [error.message || String(error)]));
				}
			});
		});
	}
